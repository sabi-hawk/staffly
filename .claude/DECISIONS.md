...
| # | Decision | Rationale |
|---|----------|-----------|
| 99 | **A BD's primary daily-work capture is a per-profile job-application count**, kept separate from the textual task summary. New `bd_job_applications` (unique per dev_profile+day); the ONLY write path is the definer `save_job_counts(work_date, counts[])` RPC, which uses `auth.uid()` and accepts a count only for a profile the caller OWNS (so a BD can't log against someone else's profile, and there's no direct-write RLS policy to spoof). Reads are owner-scoped (BD sees own; BD-Lead/admin/super see all for performance oversight). UI: a `BdJobCounts` card on the dashboard auto-lists each profile the user owns (label = `#no Name · stack`) with a count input, a live **total** (aggregate) and per-profile **segregated** chips; shown to anyone owning ≥1 profile (BD or BD-Lead), hidden otherwise. The existing rich-text task summary stays for "other work" (resumes, cover letters, LinkedIn, mentoring juniors). Detection = "profiles you own" (not role), so it naturally scales 1..N profiles and excludes admins who apply to nothing. Reporting: BD Performance (`/crm/analytics`) got a Job-applications headline stat, a per-BD column, and a segregated per-profile table. | Owner: 70-80% of BDs only post job counts against their assigned profiles daily; make that the primary experience with an at-a-glance total + breakdown for performance, while still allowing free-text for the occasional other work. Ownership-checked definer RPC keeps it tamper-proof. |

## Half-day leave, casual→unpaid fallback, backdated leave (2026-07-08)

| # | Decision | Rationale |
|---|----------|-----------|
| 100 | **Half-days for casual (+ unpaid), not annual.** Migration `0051`: `leave_requests.days_count`→`numeric(4,1)` + `half_day`/`half_period` (a half is a single date worth 0.5, DB-checked). **Casual cap relaxed** from "one *request* per month" to **1.0 day/month by total** (pending + approved), so two 0.5 requests on different days fit. **Unpaid fallback**: when a casual request exceeds the remaining allowance, `requestLeave` returns `{ needsUnpaidConfirm, casualPart, unpaidPart }` with NO insert; the apply form shows a ConfirmDialog and, on confirm, resubmits with `allowUnpaidFallback` → the shortfall is created as unpaid (a half stays a half; a multi-day range splits whole casual days first, rest unpaid — reusing `splitWorkingDates`, mirroring the annual overflow). **Backdating**: a self-filed leave may start as far back as `today−7` (`BACKDATE_DAYS`); older needs an admin (`AddLeave`). Apply form: a Duration (Full/Half) select for casual/unpaid → single Day + First/Second-half; DatePicker `min = today−7`. | Owner spec: half-day under casual, two halves per month, unpaid fallback with a proceed/cancel modal, backdate a missed day up to a week. Reused the existing overflow-split + ConfirmDialog + owner-approval pipeline so behaviour stays consistent and no auto-approve leaks in. |
