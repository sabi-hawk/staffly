  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <Section title="CRM & deals">
        <ul>
          <li>CRM Leads is one hub keyed by company (Leads cards / Interviews / Assessments), with per-lead documents, job description, BD notes, and company contacts. The Leads list opens on <strong>In progress</strong> by default, and can be viewed as <strong>Cards</strong> or a <strong>Board</strong> (Kanban pipeline): columns are stages (New / Assessment / 1st…furthest round / Selected / Closed / Parked) and each lead sits in the column of its <strong>current step</strong> (its most recent activity), so a lead whose latest activity is an assessment shows under Assessment even if it already did Round 1. Placement is automatic (no drag-drop); horizontal scroll for many rounds.</li>
          <li><strong>Add an interview to Google Calendar in one click</strong> (from the interview row and the calendar popup): opens a pre-filled Google Calendar event at the interview&apos;s <strong>Pakistan time</strong>, with the <strong>assigned developer added as a guest</strong> (their primary email), the <strong>meeting link</strong> as the location, and the lead&apos;s <strong>documents as time-limited links</strong> in the description. The BD reviews and hits Save. (Actual file <em>attachments</em> on the event need the Google Calendar API + OAuth and are a planned follow-up; for now the documents are links.)</li>
          <li><strong>Job Hunt Board</strong> (<code>/crm/job-board</code>): a shared, <strong>live</strong> board where BDs log hunted job posts (company, position, job-post link + copy, stack). Every field is optional. All BDs see every row in real time; own rows are tagged <strong>You</strong>. <strong>Repeated companies are colour-coded</strong> by how many BDs added them; <strong>bulk-paste</strong> many URLs at once (de-duped vs the paste and the board); filter by BD / company / position; expand a row to edit (owner edits all fields; any BD edits the shared <strong>feedback</strong> and can <strong>dismiss</strong> a post with a reason, kept struck-through for the record). The board <strong>auto-cleans</strong>: a daily job removes posts older than 10 days so it stays fast, but each BD&apos;s <strong>daily &ldquo;jobs hunted&rdquo; count is snapshotted first and kept forever</strong>, so the daily summary and admin attendance report stay accurate for past dates.</li>
          <li><strong>Assessments</strong> can be tagged (all optional) with a <strong>Camera</strong> setting (with camera / without camera / not determined) and a <strong>Category</strong> — the kind of assessment (Coding, MCQs, Coding + MCQs, Video introduction, …). The category list is <strong>configurable</strong>: &ldquo;Manage categories&rdquo; on the Assessments tab lets a CRM manager add, rename, or deactivate categories on the fly, the same way stacks are managed. Both fields show as columns and filters on the Assessments grid.</li>
          <li>A <strong>deal</strong> records a <strong>Closer</strong> (who landed it), a <strong>primary BD owner</strong> and an optional <strong>secondary BD owner</strong>, and one or more <strong>working members</strong> — <strong>developers or designers</strong> (a deal can be design work, not just development). The CRM <strong>lead is optional</strong> (deals can predate the portal). Each deal has a unique <strong>4-digit code</strong>, so a company with several deals stays unambiguous, and a <strong>currency</strong> (PKR / USD / EUR / GBP / AUD / …) for its amount. The list has <strong>Edit</strong> and <strong>Delete</strong> per profile.</li>
          <li><strong>Deals roll up by company:</strong> the deals list groups several deals under one company record showing its deal count and a status summary; expand a company to see all its deals. When creating or editing a deal, the <strong>Company picker</strong> lets you reuse an existing company (which groups the deals) or add a new one. A deal that was named inconsistently can be re-pointed by editing it and choosing the right company.</li>
          <li><strong>Two BD owners per deal:</strong> the <strong>primary</strong> is the direct BD; the <strong>secondary</strong> is typically the BD-Lead who trained the primary and, per company policy, earns a commission on that junior's deal too. Both are paid via their <strong>own deal-commission %</strong> (set on each one's employee page), so the two can differ. The BD-owner filter finds deals where a BD is either the primary or the secondary owner.</li>
      ...
      <Section title="Payroll">
        <ul>
          ...
          <li>Payroll generates for <strong>anyone payable</strong> — a base salary, active deal commissions, or recurring compensation — so a <strong>commission-only partner</strong> (no base pay) still gets a payslip whose net is their commissions. An <strong>Exclude-from-payroll</strong> flag skips a specific person entirely (e.g. the founder).</li>
          <li><strong>Deal commissions:</strong> a BD <strong>or a deal-assigned developer</strong> can be paid a <strong>% of a deal&apos;s receipts</strong> (or a fixed one-off) on top of their base salary, set on their profile. Each deal keeps a <strong>payments ledger</strong>: every receipt is logged with its receiving date and the <strong>billing month</strong> it counts toward (which can differ, e.g. money that arrives in August but belongs to July still lands on July&apos;s payslip). When payroll runs, the commission = rate × the deal&apos;s receipts billed to that period. On the <strong>payslip document</strong> commissions are shown as one aggregated <strong>&ldquo;Deal commissions&rdquo;</strong> total (a BD never sees which deal earned what); the <strong>admin</strong> sees the per-deal breakdown in the payroll row expand.</li>
          <li><strong>Each run expands</strong> to its full breakdown (base, missing-attendance, every commission/addition/deduction). Any line can be <strong>dismissed</strong> — struck through, kept for the record, excluded from the total and the payslip (useful when an attendance deduction is judged invalid) — and restored later. <strong>Recompute</strong> re-runs one employee&apos;s calculation for the period so the missing-attendance deduction shrinks as days pass. A <strong>catch-up deal commission</strong> can be added for a previous month (a direct amount, or the stored rate × that month&apos;s receipts).</li>
